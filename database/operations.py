...
from database.models import MatchURL, Match, Lineup, Map, MapURL
...
def get_unscraped_date_range():
	...
	return max_date.split(" ")[0], datetime.now().strftime("%Y-%m-%d")
...
def insert_map_urls(map_url_list):
	...
	for map_url_dict in map_url_list:
		...
		try:
			...
			db_logger.error(f"{map_url['id']} already exists in DB")
			continue
		count += 1
	session.commit()
	db_logger.info(f"{count} MapURLs added to database")